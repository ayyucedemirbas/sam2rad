...
if __name__ == "__main__":
    ...
    class HFModel(PreTrainedModel):
        ...
        def state_dict(self, *args, **kwargs):
            orig_state = self.model.state_dict(*args, **kwargs)
            cloned_state = {k: v.clone() for k, v in orig_state.items()}
            return cloned_state

    hf_model = HFModel(learner.model)


    hf_model.save_pretrained("sam2rad-hf", safe_serialization=True)
    print("Model saved in safetensors format in the 'sam2rad-hf' directory.")


    hf_model.push_to_hub(
        repo_id="ayyuce/sam2rad",
        use_temp_dir=False,
        commit_message="trained for 10 epochs",
        token="hf_xxx",
        use_safetensors=True,
    )
    hf_model.push_to_hub(
        repo_id="MLforHealthcare/sam2rad",
        use_temp_dir=False,
        commit_message="trained for 10 epochs",
        token="hf_xxx",
        use_safetensors=True,
    )
    print("Model pushed to Hugging Face Hub as ayyuce/sam2rad and MLforHealthcare/sam2rad.")
